...
def load_selected_indices(args):
    ...
    selected_indices = df_selected["index"].to_numpy()
    labels = df_selected["label"].to_numpy()
    
    positive_indices = np.random.RandomState(seed=args.seed).permutation(np.where(labels == 1)[0])
    negative_indices = np.random.RandomState(seed=args.seed).permutation(np.where(labels == 0)[0])

    if len(positive_indices) >= args.k // 2:
        positive_indices = positive_indices[:args.k//2]
    negative_indices = negative_indices[:args.k-len(positive_indices)]

    positive_indices = selected_indices[positive_indices]        
    negative_indices = selected_indices[negative_indices]

    selected_indices = np.concatenate([positive_indices, negative_indices], axis=0).tolist()
...
def load_in_context_examples(args):
    ...
    return example_inputs, example_labels, example_embeddings, selected_indices
...
def initialization(args):
    ...
    example_inputs, example_labels, example_embeddings, selected_indices = load_in_context_examples(args)
    ...
    return model_name, model, tokenizer, test_prompts, test_labels, selected_indices


if __name__ == "__main__":
    torch.cuda.empty_cache()
    args = parse_args()
    output_file = f"results/results_{args.version}/{args.dataset}/{args.seed}/{args.selection_method}_{args.lm}.csv"
    if os.path.exists(output_file):
        print("File exists")
        exit()

    (model_name, model, tokenizer, test_prompts, test_labels, selected_indices) = initialization(args)
    ...
    df_result = pd.DataFrame({"label": test_labels, "pred": preds, "prob": probs, "selected_indices": [selected_indices]*len(test_labels)})
    if os.path.exists(f"results/results_{args.version}/{args.dataset}/{args.seed}") is False:
        os.makedirs(f"results/results_{args.version}/{args.dataset}/{args.seed}")
